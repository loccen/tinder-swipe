...
class TelegramCollector:
    ...
    MAGNET_PATTERN = re.compile(
        r"magnet:\?xt=urn:[a-z0-9]+:[a-zA-Z0-9]{32,}[^\s]*", re.IGNORECASE
    )
    PIKPAK_PATTERN = re.compile(
        r"https?://mypikpak\.com/s/[A-Za-z0-9_-]+", re.IGNORECASE
    )
    ...
    def __init__(self):
        ...
        # 从数据库读取频道列表，初始化为空
        self.channels: Set[Union[str, int]] = set()
    ...
    async def start(self):
        ...
        # 从数据库加载频道列表
        self.channels = await self._load_channels_from_db()
        logger.info(f"从数据库加载到 {len(self.channels)} 个监听频道")
    ...
    async def _flush_pending_resources(self):
        ...
        for chat_id in list(self._recent_media.keys()):
            self._recent_media[chat_id] = [
                m
                for m in self._recent_media[chat_id]
                if (now - m.created_at).total_seconds() < 60  # 保留1分钟内的
            ]

    async def _load_channels_from_db(self) -> Set[Union[str, int]]:
        """从数据库加载频道列表"""
        try:
            async with async_session_maker() as session:
                ...
                for ch in raw_channels:
                    ...
                    if isinstance(ch_id, str) and ch_id.lstrip("-").isdigit():
                        channels.add(int(ch_id))
                    else:
                        channels.add(str(ch_id))

                logger.debug(f"从数据库加载频道: {list(channels)}")
                return channels

        except json.JSONDecodeError as e:
            logger.error(f"解析频道配置 JSON 失败: {e}")
            return set()
        except Exception as e:
            logger.error(f"从数据库加载频道配置失败: {e}", exc_info=True)
            return set()

    async def _reload_channels(self):
        """从数据库重新加载频道配置"""
        try:
            new_channels = await self._load_channels_from_db()
        ...
        try:
            ...
            try:
                ...
                if not self._is_monitored_channel(chat_id, username):
                    logger.debug(
                        f"跳过非监听频道消息: chat_id={chat_id}, username={username}"
                    )
                    return
            except Exception as chat_err:
                if not self._is_monitored_channel(chat_id):
                    logger.info(
                        f"获取聊天信息失败且不在直接监听列表，跳过消息: chat_id={chat_id}, error={chat_err}"
                    )
                    return
            ...
            if urls:
                logger.info(
                    f"发现资源消息: chat_id={chat_id}, msg_id={msg_id}, 包含链接数={len(urls)}"
                )
                logger.info(f"提取到的链接: {urls}")
            ...
            if urls:
                ...
                for url in urls:
                    ...
                    logger.info(
                        f"任务已加入待处理队列 (等待关联更多分片): {title[:30]}..., 当前预览图: {len(preview_images)}张, key={resource_key}"
                    )
    ...
    def _get_recent_images(self, chat_id: int) -> List[str]:
        ...
        for media in self._recent_media.get(chat_id, []):
            if (
                media.image_path
                and (now - media.created_at).total_seconds() < self.ASSOCIATION_WINDOW
            ):
    ...
    async def _try_associate_media(self, chat_id: int, media: RecentMedia):
        ...
        for key, resource in self._pending_resources.items():
            if resource.chat_id == chat_id:
                # 检查时间窗口
                if (
                    abs((media.created_at - resource.created_at).total_seconds())
                    < self.ASSOCIATION_WINDOW
                ):
                    if (
                        media.image_path
                        and media.image_path not in resource.preview_images
                    ):
                        resource.preview_images.append(media.image_path)
                        logger.info(f"关联预览图到资源: {resource.title[:20]}...")
    ...
    async def _submit_resource(self, resource: PendingResource):
        ...
        try:
            ...
            payload = {
                ...
                "preview_image": (
                    resource.preview_images[0] if resource.preview_images else None
                ),
            ...
            logger.info(
                f"任务创建成功: {resource.title[:30]}... (预览图: {len(resource.preview_images)}张)"
            )
    ...
    async def _download_preview(
        self, message, chat_id: int, msg_id: int
    ) -> Optional[str]:
        """下载消息中的预览图"""
        try:
            media = message.media
            ...
            await self._client.download_media(message, file=str(filepath), thumb=-1)
